...
SUMMARY_TITLE_FONT_PATH = "./font/courbd.ttf"
...
def summary_and_tiles(slide_num, display=True, save_summary=False, save_data=True, save_top_tiles=True):
  ...
  tile_sum,high = score_tiles(slide_num, np_img)
  if save_data:
...
def singleprocess_filtered_images_to_tiles(display=False, save_summary=True, save_data=True, save_top_tiles=True,
  ...
  if image_num_list is not None:
    ...
    image_num_list, tile_summaries_dict = image_range_to_tiles(0, num_training_slides, display, save_summary, save_data,
                                                               save_top_tiles)
  ...
  for row in range(num_rows):
    ...
    for col in range(num_cols):
      ...
      if tile_num <= num_tiles:
        ...
        tile_img_path = slide.get_tile_image_path(t,slide_num)
...
def dynamic_tiles(slide_num, small_tile_in_tile=False):
  ...
  tile_summary,high = score_tiles(slide_num, filt_np_img, (large_w, large_h, small_w, small_h), small_tile_in_tile)
  return tile_summary
...
#   tile = dynamic_tile(2, 14, 16, True)
