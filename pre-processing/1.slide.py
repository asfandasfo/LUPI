...
#  slide_filepath = os.path.join(SRC_TRAIN_DIR, TRAIN_PREFIX + padded_sl_num + "." + SRC_TRAIN_EXT)
  ...
  slides=glob.glob('/home/fayyaz/Desktop/AsfandYaar/FYP_AsfandYaar/Image_data/slides/'+MODE+'/*TS1*.svs')[1:2]
...
def get_tile_image_path(tile,slide_num):
  ...
  """
  t = tile
  padded_sl_num = get_training_slide_path(slide_num).split('/')[-1][:23]
...
def get_tile_image_path_by_slide_row_col(slide_number, row, col):
  ...
  Returns:
    Path to image tile.
  """
  padded_sl_num = get_training_slide_path(slide_number).split('/')[-1][:23]
...
def get_num_training_slides():
  ...
  """
  num_training_slides = len(glob.glob('/home/fayyaz/Desktop/AsfandYaar/FYP_AsfandYaar/Image_data/slides/'+MODE+'/*TS1*.svs')[1:2])
...
def singleprocess_training_slides_to_images():
  ...
  training_slide_range_to_images(0, num_train_images)
...
def slide_stats():
  ...
  num_train_images = get_num_training_slides()
  slide_stats = []
  for slide_num in range(num_train_images):
...
#    slide_stats()
#    training_slide_to_image(0)
#    img_path = get_training_image_path(0)
#    img = open_image(img_path)
#    img.show()
#
#    slide_to_scaled_pil_image(0)[0].show()
